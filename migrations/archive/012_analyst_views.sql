-- 012_analyst_views.sql
--
-- Analyst/warehouse convenience views.
--
-- This is intentionally “best-effort” and spreadsheet-friendly.
-- It favors wide, one-row-per-player outputs like Sean’s X/Y warehouses.

BEGIN;

-- -----------------------------------------------------------------------------
-- 1) Active contract + latest version per player
--
-- Definition:
-- - contracts.record_status_lk IN ('APPR','FUTR')
-- - pick best single contract per player (APPR over FUTR, then signing_date)
-- - pick latest version_number within that contract
-- -----------------------------------------------------------------------------

CREATE OR REPLACE VIEW pcms.vw_active_contract_versions AS
...
SELECT
  ...
  ac.signing_team_id,
  ac.team_code,
  ac.record_status_lk AS contract_status_lk,
...
-- 2) Salary pivot (wide)
--
-- This is the core “warehouse” pivot other views can reuse.
-- -----------------------------------------------------------------------------

CREATE OR REPLACE VIEW pcms.vw_salary_pivot_2024_2030 AS
SELECT
  s.contract_id,
  s.version_number,
  ...
  SUM(CASE WHEN s.salary_year >= 2024 THEN s.total_salary ELSE 0 END) AS total_salary_from_2024,
...
-- -----------------------------------------------------------------------------
-- 3) Y Warehouse view (forward-looking, 2025–2030)
--
-- Notes:
-- - One row per *active* player as defined by vw_active_contract_versions.
-- - Includes NBA + DLG people rows; in the current DB, many two-way players
--   are stored under league_lk='DLG'.
