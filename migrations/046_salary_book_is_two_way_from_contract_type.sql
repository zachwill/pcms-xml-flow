-- 046_salary_book_is_two_way_from_contract_type.sql
--
-- DEPRECATED / SUPERSEDED:
-- This migration was an initial attempt to fix salary_book_warehouse.is_two_way,
-- but it used the simplified refresh function and unintentionally regressed the
-- multi-contract per-year salary selection logic (see 034).
--
...
-- - derive two-way from contract_type_lk
...
BEGIN
  ...
  ac AS (
    SELECT
      ...
      -- IMPORTANT:
      -- contract_versions.is_two_way is not reliably populated by the source/ingest.
      -- Derive from contract type instead.
      (lv.contract_type_lk = '2WCT') AS is_two_way,
