-- 034_salary_book_multi_contract_year_selection.sql
--
-- Fix salary_book_warehouse salary grids for players with multiple relevant contracts.
--
-- Problem:
-- - The original refresh picked ONE contract per player (preferring APPR over FUTR).
-- - This can miss future-year salary lines from a newer FUTR contract that supersedes
--   a declined option year on the older APPR contract.
--   Example: Joel Embiid (203954) has APPR contract 77045 with a 2026 player option
--   declined (option_decision_lk='POD'), and a newer FUTR contract 99163 providing
--   2026+ salaries. The warehouse should show 2026+ from 99163.
--
-- Approach:
-- - Keep selecting a single "primary" contract per player for identity/flags.
-- - But select salary rows PER (player_id, salary_year) across *all* APPR/FUTR contracts,
--   excluding option years where the option was declined.
-- - Pivot those selected per-year rows into the warehouse cap/tax/apron grids.
--
-- Notes:
-- - We detect declined options via pcms.lookups (lookup_type='lk_option_decisions'
--   with description ILIKE '%Declined%').
--
