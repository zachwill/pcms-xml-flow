...
const VALIDATION_MARKER = "<!-- NBA_SCHEMA_VALIDATED -->";
const TASK_FILE = ".ralph/NBA_SCHEMA.md";
...
function hasPlanningComplete(): boolean {
  try {
    ...
    // Planning is complete if we have DDL generation items
...
loop({
  ...
  supervisor: supervisor(
    `
    Review the generated DDL in \`f/ralph/new_nba_schema.flow/\`.

    Cross-reference against:
    - \`backlog/ralph/nba/AGENTS.md\` — design philosophy, naming conventions
    - \`backlog/ralph/nba/schema/*.txt\` — source specifications
    ...
    - Are naming conventions followed (snake_case, nba_id for players, team_id for teams)?
    - Are statistical columns using short abbreviations (fgm, fga, fg_pct, etc.)?
    - Is JSONB used appropriately (pbp, complex nested data)?
    ...
    - Commit: git add -A && git commit -m "nba-schema: supervisor review"
    `,
  ...
  run(state) {
    ...
    if (!state.hasTodos && hasValidationMarker()) {
      return halt("NBA schema DDL generation complete");
    }
    ...
    if (state.hasTodos) {
      return work(
        `
        You are generating Postgres DDL for the NBA schema.

        Your task: ${state.nextTodo}

        ## Required Reading (if not already familiar)

        1. \`backlog/ralph/nba/AGENTS.md\` — design philosophy, naming conventions, provenance requirements
        2. \`backlog/ralph/nba/schema/*.txt\` — table specifications to convert
        3. \`backlog/ralph/nba/data/\` — data pipeline context (if exists)

        ## Output

        Write DDL to \`f/ralph/new_nba_schema.flow/inline_script_*.pg.sql\`

        ## DDL Conventions

        - First file must include: \`CREATE SCHEMA IF NOT EXISTS nba;\`
        - Use \`SET search_path TO nba;\` after schema creation
        - No FK constraints — add comments like \`-- FK: references teams(team_id)\`
        ...
        - If creating a new inline_script file, update \`f/ralph/new_nba_schema.flow/flow.yaml\`
        - Add the new module following the existing pattern

        When done:
        - Check off ONLY this task in \`${TASK_FILE}\`
        - git add -A && git commit -m "nba-schema: <summary>"
        - Exit immediately.
        `
      );
    }
    ...
    if (!hasAnyChecklistItems() || !hasPlanningComplete()) {
      return generate(
        ...
        1. \`backlog/ralph/nba/AGENTS.md\` — design philosophy, conventions
        2. ALL files in \`backlog/ralph/nba/schema/*.txt\` — table specifications
        3. \`backlog/ralph/nba/data/\` — data pipeline context (if exists)
        4. \`backlog/ralph/nba/docs/\` — API endpoint documentation

        ## Step 2: Plan File Split

        Group tables logically and estimate LOC. Target ~400-500 LOC per file.
        Consider: core entities (teams, players) → game data → stats → ngss_* tables
        ...
        \`\`\`markdown
        # NBA Schema Generation

        **Output directory:** \`f/ralph/new_nba_schema.flow/\`

        ## Phase 1: Planning
        - [x] Read backlog/ralph/nba/AGENTS.md
        - [x] Read all backlog/ralph/nba/schema/*.txt
        - [x] Read backlog/ralph/nba/data/ and docs/
        - [x] Plan file split
        ...
        Commit: git add -A && git commit -m "nba-schema: create generation plan"
        `,
        { thinking: "high" }
      );
    }
    ...
    return work(
      ...
      For EVERY file in \`backlog/ralph/nba/schema/*.txt\`:
      - Confirm there's a corresponding CREATE TABLE in \`f/ralph/new_nba_schema.flow/\`
      - Verify column names and types match the spec
      - Verify constraints (PK, UNIQUE) are present

      ### Step 2: Convention Compliance

      Check against \`backlog/ralph/nba/AGENTS.md\`:
      - Provenance columns: source_api, source_endpoint, fetched_at, source_hash
      - Naming: snake_case, nba_id for players, team_id for teams
      - Statistical columns: fgm, fga, fg_pct, fg2m, fg2a, etc.
      - JSONB usage: appropriate for PBP and complex nested data
      ...
      If gaps found:
      - Add tasks to \`${TASK_FILE}\`
      - Commit: git add -A && git commit -m "nba-schema: validation found gaps"

      If complete:
      - Add validation summary to \`${TASK_FILE}\`
      - Add marker: \`${VALIDATION_MARKER}\`
      - Commit: git add -A && git commit -m "nba-schema: validation complete"
      `,
