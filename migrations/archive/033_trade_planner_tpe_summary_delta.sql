-- 033_trade_planner_tpe_summary_delta.sql
--
-- Add a simple UI convenience field to the planner summary:
-- - summary.main_leg_max_incoming_cap_delta = max_incoming_cap - main_leg_incoming_total_cap
--
-- Notes:
-- - We keep migrations append-only. Even though this is small, editing an already-run
--   migration can cause schema drift across environments.
...
DECLARE
  ...
  v_main_leg_max_incoming_cap_delta bigint;
...
BEGIN
  ...
  v_main_leg_max_incoming_cap_delta := NULL;
  ...
  IF v_max_incoming_cap IS NOT NULL THEN
    v_main_leg_max_incoming_cap_delta := GREATEST(0::bigint, v_max_incoming_cap - v_main_leg_incoming_total_cap);
  END IF;
  ...
  v_result := jsonb_build_object(
    ...
    'summary', jsonb_build_object(
      ...
      'main_leg_max_incoming_cap_delta', v_main_leg_max_incoming_cap_delta
    ),
