-- 086_team_salary_warehouse_two_way_count_from_salary_book_yearly.sql
--
-- Problem:
-- - pcms.team_salary_warehouse.two_way_row_count was sourced from
--   pcms.team_budget_snapshots budget_group_lk='2WAY' row count.
-- - team_budget_snapshots can include stale/non-active two-way rows,
--   causing impossible team counts (e.g. 4 two-way contracts).
--
-- Fix:
-- - For years covered by pcms.salary_book_yearly, derive two_way_row_count
--   from distinct players flagged is_two_way.
-- - For any fallback years not covered by salary_book_yearly, keep the
--   snapshot-derived count but clamp to NBA max slots (3).
...
BEGIN
  ...
  two_way_contract_counts AS (
    SELECT
      sby.team_code,
      sby.salary_year,
      COUNT(DISTINCT sby.player_id) FILTER (WHERE COALESCE(sby.is_two_way, false))::int AS two_way_row_count
    FROM pcms.salary_book_yearly sby
    WHERE sby.team_code IS NOT NULL
      AND sby.salary_year IS NOT NULL
    GROUP BY 1,2
  ...
  SELECT
    ...
    -- Prefer salary_book_yearly two-way players when available.
    -- Fallback to snapshot count (clamped to NBA max slots: 3).
    COALESCE(twcc.two_way_row_count, LEAST(3, a.two_way_row_count))::int AS two_way_row_count,
  ...
  LEFT JOIN two_way_contract_counts twcc
    ON twcc.team_code = a.team_code
   AND twcc.salary_year = a.salary_year
