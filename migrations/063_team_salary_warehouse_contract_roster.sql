-- 063_team_salary_warehouse_contract_roster.sql
--
-- Clarify roster cap totals:
-- - cap_rost now reflects contract-only roster salary (from salary_book_yearly).
-- - cap_rost_hold preserves the budget snapshot "ROST" bucket (includes holds/rights).

BEGIN;

ALTER TABLE pcms.team_salary_warehouse
  ADD COLUMN IF NOT EXISTS cap_rost_hold bigint;
