-- 043_salary_book_player_consent_flags.sql
--
-- Add player consent / trade-consent related fields to salary_book_warehouse.
--
-- Background:
-- - contract_versions.version_json contains `player_consent_lk` (NONE/YEARK/YRKPC/ROFRE)
-- - Some consents are time-bounded (e.g. ROFRE is a 1-year restriction after matching)
--
-- We store:
-- - the raw code + end date
-- - a couple of convenience booleans for tool queries
--
...
-- Update refresh function to populate these fields.
-- Most recent definition lives in 034_salary_book_multi_contract_year_selection.sql.
-- We keep the logic here so the DB has the latest function after applying migrations.
