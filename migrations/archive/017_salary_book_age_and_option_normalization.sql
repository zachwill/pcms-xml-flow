-- 017_salary_book_age_and_option_normalization.sql
--
-- Adjustments based on tool needs:
-- - Age should be a decimal (e.g. 31.9), not an integer.
-- - Option columns should not mix NULL vs 'NONE'. We normalize 'NONE' -> NULL.
--
-- Note: salary_book_warehouse is refreshed via TRUNCATE/INSERT, so the refresh
-- function definition is the main place to enforce normalization.

BEGIN;

-- Age should be a decimal.
ALTER TABLE pcms.salary_book_warehouse
  ALTER COLUMN age TYPE numeric(4,1);

-- Replace refresh function with:
-- - age as decimal years (1 decimal place)
-- - option_*: NULLIF(value,'NONE')
...
BEGIN
  ...
  SELECT
    ...
    -- Decimal age (1 decimal place). Example: 31.9
    CASE
    ...
    -- Normalize option_lk: treat 'NONE' same as NULL.
