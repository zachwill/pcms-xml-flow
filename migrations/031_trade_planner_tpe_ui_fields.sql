-- 031_trade_planner_tpe_ui_fields.sql
--
-- Add UI-friendly convenience fields to pcms.fn_trade_plan_tpe(...) output.
--
-- Adds:
-- - absorption_legs[].absorbed_count
-- - absorption_legs[].remaining_pct (remaining_amount_after / remaining_amount_before)
-- - main_leg.max_incoming_cap (alias of tpe_math.max_replacement_salary)
-- - main_leg.created_tpe_cap (alias of tpe_math.created_exception_amount)
...
BEGIN
  ...
  v_result := jsonb_build_object(
    ...
    'main_leg', jsonb_build_object(
      ...
      'tpe_math', v_tpe_math,
      'max_incoming_cap', NULLIF((v_tpe_math->>'max_replacement_salary')::bigint, NULL),
      'created_tpe_cap', NULLIF((v_tpe_math->>'created_exception_amount')::bigint, NULL)
    )
  );
