-- 051_team_salary_warehouse_exclude_two_way_from_roster_count.sql
--
-- Fix roster_row_count to use authoritative source: team_two_way_capacity.current_contract_count
-- Also add other useful columns from team_two_way_capacity to make this a one-stop shop.
--
-- New columns from team_two_way_capacity:
--   - games_remaining
...
ALTER TABLE pcms.team_salary_warehouse 
  ADD COLUMN IF NOT EXISTS games_remaining integer,
  ADD COLUMN IF NOT EXISTS under_15_games_count integer,
  ADD COLUMN IF NOT EXISTS under_15_games_remaining integer;
