...
--
-- Restore the multi-contract per-year salary selection logic introduced in
-- 034_salary_book_multi_contract_year_selection.sql (critical for cases like
-- Embiid: declined option year + FUTR extension) while also fixing is_two_way.
--
-- Fix:
-- - pcms.contract_versions.is_two_way is not populated; derive is_two_way from
--   contract_type_lk IN ('2WCT','REGCV') per pcms.lookups.
...
BEGIN
  ...
  ac AS (
    ...
    SELECT
      ...
      -- FIX: derive two-way from contract type (2WCT or converted two-way REGCV)
