-- 053_salary_book_contract_type_metadata.sql
--
-- Add contract type metadata to pcms.salary_book_warehouse so the UI can display
-- a short code (e.g. RKSC, VETEX) and a canonical human-readable label from
-- pcms.lookups (lk_contract_types).
--
-- Implementation notes:
-- - Warehouse stores BOTH code + label (no UI joins needed).
-- - Values are sourced deterministically from the chosen active contract version
